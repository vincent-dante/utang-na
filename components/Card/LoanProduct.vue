...
<template>
  <div
    class="flex w-full flex-wrap overflow-hidden rounded-2xl border bg-white p-5"
  >
    ...
    <p class="truncate pb-3 text-sm font-bold">
      {{ product.lender }}
    </p>
    <p class="pb-3 text-xs text-gray-600">Credit limit</p>

    <p class="overflow-hidden text-3xl font-bold">
      {{ formatToCurrencyPhp(product.min_amt) }} to
      {{ formatToCurrencyPhp(product.max_amt) }}
    </p>
    <p class="my-3 font-bold">{{ termRange }}</p>
    ...
    <div
      class="mb-3 inline-block rounded-full border border-lavander-400 px-2 py-1 text-xs text-lavander-400"
    >
      {{ product.loan_type }}
    </div>
